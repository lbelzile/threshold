# Wan and Davis bivariate radial threshold selection method
#
...
WD.diag <- function(
  ...
  if (margtransfo == "Pareto") {
    pseudo <- 1 /
      (1 - apply(dat, 2, rank, ties.method = ties.method) / (nrow(dat) + 1L)) -
      1
  } else if (margtransfo == "Frechet") {
    pseudo <- -1 /
      log(apply(dat, 2, rank, ties.method = ties.method) / (nrow(dat) + 1L))
  }
  ...
  # 2) form pseudo-observations (radius and angles)
  if (norm == "proj") {
    stopifnot(int <= ncol(pseudo), int > 0)
    radius <- pseudo[, int]
  ...
  for (i in seq_along(u)) {
    pval[i] <- energy::indep.test(
      x = log_radius[log_radius > u[i]],
  ...
  if (plot) {
    ...
    lines(x = 1 - thresh, y = rev(pval_chpt$est), col = 2)
